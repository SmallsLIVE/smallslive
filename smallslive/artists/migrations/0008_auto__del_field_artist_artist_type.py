...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Artist.artist_type'
        db.delete_column(u'artists_artist', 'artist_type_id')

        # Rename the dummy m2m table to the "real" name
        db.rename_table(u'artists_artist_artist_type_many', u'artists_artist_artist_type')

    def backwards(self, orm):
        # Adding field 'Artist.artist_type'
        db.add_column(u'artists_artist', 'artist_type',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['artists.ArtistType'], null=True, blank=True),
                      keep_default=False)

        # Adding M2M table for field artist_type_many on 'Artist'
        db.rename_table(u'artists_artist_artist_type', u'artists_artist_artist_type_many')

    models = {
        u'artists.artist': {
            'Meta': {'ordering': "['last_name']", 'object_name': 'Artist'},
            'artist_type': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'related_name': "'artists'", 'blank': 'True', 'to': u"orm['artists.ArtistType']"}),
