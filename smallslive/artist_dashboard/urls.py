...
urlpatterns = patterns('artist_dashboard.views',
    ...
    url(r'^generate-payouts/(?P<period_start>\d{4}-\d{2}-\d{2})/(?P<period_end>\d{4}-\d{2}-\d{2})/(?P<total>\d+)/(?P<deductable>\d+)/(?P<foundation_total>\d+)/(?P<foundation_deductable>\d+)/(?P<projects_total>\d+)/(?P<projects_deductable>\d+)/(?P<events_total>\d+)/(?P<events_deductable>\d+)$', 'metrics_payout', name='metrics_payout'),
